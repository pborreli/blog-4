...
comments: true
categories: erlang, oo, object-oriented, functional
---
...
## Functional Programming and Object Orientiation
...
first class functions which are not immcompatable with object orientation.
...
Such terms feel very familiar to Java and Ruby programmers. However, to truely
...
OO is only about message passing and issolation and protection of state.

Concepts such as classes and inheritance are only an attempt to implement the
ideas above. The over emphasis on classes and objects at the expense of message
passing has led many "OO" languages away from the core concepts of object
...
If we take Alan Kay's definition of oject orientation as canonical, then Erlang
fits almost perfectly. Instead of "objects" persay, Erlang uses "processes" that
are fully isolated contructs that can only communicate with each through message passing.

Unlike objects in Java, Ruby or other so-called "object oriented" languages,
Erlang processes are fully isolated from one another, so much so that catstropic failure
...
Erlang is perhaps the only language that truely embraces the ideas that Kay
puts forth as essential to object orientation: message passing and issolation.
